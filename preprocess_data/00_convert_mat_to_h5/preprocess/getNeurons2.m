function [InfoNeuron2, SpikeTimes2]=getNeurons2(nev)
%idx_valid=find(~ismember(nev(:,2),[0,255]));
idx_valid=find(nev(:,1)>0 & ~ismember(nev(:,2),[0,255]));

DUR = max(nev(:,3))-min(nev(:,3));
...
for u=1:length(UNITS)
    ...
    for n=1:length(NEURONS)
        ...
        % Reject neurons that spike at < 0 Hz
        fprintf('CHANNEL: %d \t NEURON: %d \t SPIKES: %d\n', unit, neuron, length(idx_neu));
        if length(idx_neu)/DUR >= 0
            Nneuron=Nneuron+1;
